...
\begin{frame}{Cos'è la costruzione NPN}
  ...
  in particolare da \cite{jackendoff2008construction} %Jackendoff (2008)
  e \cite{sommerer2021absent} %Sommerer \& Baumann (2021).
  ...
  La costruzione non ammette nomi accompagnati da determinanti.
\end{frame}

\begin{frame}{Cos'è la costruzione NPN}
  La costruzione può comparire in diverse funzioni sintattiche, ad esempio come modificatore avverbiale o come modificatore nominale.
...
Indica una \textbf{successione o iterazione},
spesso temporali o spaziali.
  \end{block}

  \begin{block}{JUXTAPOSITION}
Indica una \textbf{giustapposizione o confronto},
spesso persone o oggetti.
  \end{block}

\end{frame}


\begin{frame}{Obiettivo dello studio}

L'obiettivo dello studio è valutare se i Large Language Models, come BERT, siano in grado di rappresentare, almeno in parte,
l'unità fondamentale della \textbf{Construction Grammar}: la \textbf{costruzione}.

In particolare, il case study si concentra sulla costruzione inglese \textbf{NPN} instanziata da \textit{to}, scelta per la sua \textbf{polisemia} e i molteplici significati contestuali.
...
\begin{alertblock}{Non conosciamo}
\begin{itemize}
...
\begin{frame}{Tipologie di modelli Transformer}
\begin{center}
    \includegraphics[width=1\textwidth]{images/transformer.png}
    \\[2mm]
    \small Modelli Encoder/Decoder
\end{center}
\end{frame}
...
\begin{frame}{Rappresentazione vettoriale in BERT}
\begin{center}
    \includegraphics[width=1\textwidth]{images/bert.png}
    \\[2mm]
    \small Embedding contestuali
\end{center}
\end{frame}
...
\begin{block}{Probing}
  Esaminare i \textbf{vettori} per analizzare se contengono determinate conoscenze linguistiche
\end{block}
...
\begin{frame}{Che cos'è un classificatore?}

\textbf{Definizione}: un classificatore è un modello che assegna
un'istanza (nel nostro caso un embedding contestuale)
a una \textbf{classe} tra un insieme finito di categorie.



\textbf{Obiettivo}:
\[
f:\mathbf{x} \mapsto y \in \{1, \dots, K\}
\]

\textbf{Fase di Apprendimento (Training)} \\
Il modello viene addestrato su un \textbf{training set},
cioè esempi etichettati:
\[
(\mathbf{x}_i, y_i)
\]
per stimare i parametri che distinguono le classi.



\textbf{Fase di Valutazione (Test)} \\
Un \textbf{test set} separato misura quanto il modello
generalizza su dati mai visti.
...
\begin{tikzpicture}[every node/.style={align=center}]
  ...
  % Box secondari sotto
  \node (left) [draw, rounded corners, fill=green!10, minimum width=3.5cm, minimum height=1cm, below left=2cm and -2cm of head] {Obiettivo migliorare \\ le performance non \\
  interrogare il modelo};
...
\begin{itemize}
  \item Analizzare le performance degli \textbf{embedding per tutti i layer} del modello. $\rightarrow$ Ci attendiamo incremento e differenza.
...
\begin{frame}{Il Dataset}
  \begin{itemize}
    \item Estrazione da COCA (Corpus of Contemporary American English)
...
\begin{frame}{Task 1: Identificazione}
  ...
  Distinguere le istanze autentiche della costruzione NtoN dagli esempi negativi del corrispondente pattern distrattore.
  \end{block}
\end{frame}

\begin{frame}{Base-line}
  \begin{block}{Control classifier}
Le etichette vengono randomizzate e assegnate in modo deterministico al \textbf{word type}.

Le performance dovrebbero attestarsi near chance.
  \end{block}
  \begin{block}{Non-contextual baseline (GloVe)}
Valuta la performance basata solo su informazioni lessicali, senza contesto (embedding statici).

Dovrebbero attendersi, in virtù dei campi semnatici ricorrenti nella costruzione NtoN come espressioni temporali e parti del corpo, performance non trascurabili.
  \end{block}
\end{frame}

\begin{frame}{A cosa serve la baseline?}
  \begin{block}{Control classifier}
Informa sullla bontà del classificatore, verificare che il classificatore non sia troppo potente.
  \end{block}
  \begin{block}{Non-contextual baseline (GloVe)}
...
\begin{center}
    ...
    \\[2mm]
    \small This shows that overall, the probing classifier seems to be picking up on some sort of information in BERT which can reliably distinguish the NtoN construction from its near minimal pair NtoN distractor counterparts, beyond what is possible through lexical semantic clues alone.
\end{center}
\end{frame}
...
\begin{itemize}
    ...
    \item Se è sensibile al \textbf{pattern N + to + N}, riconoscerà occorrenze perturbate da reali istanze della costrzione.
\end{itemize}
...
\textbf{Obiettivo:} Analizzare le etichette semantiche della costruzione NtoN.
...
\begin{itemize}
    \item \textbf{SUCCESSION}: frequente con nomi spaziotemporali (es. giorno per giorno, costa a costa)
    \item \textbf{JUXTAPOSITION}: frequente con parti del corpo o esseri umani (es. faccia a faccia, amico ad amico)
...
\begin{itemize}
    \item Classificatore per distinguere i sottotipi semantici:
        ...
        \end{itemize}
    \item Baseline = embedding statici
...
\begin{itemize}
    \item Creato un nuovo dataset di istanze NtoN estratte da COCA.

    \item Addestrato un \textit{linear probe} per distinguere la costruzione NtoN
    dai suoi quasi-minimal pair e da versioni con ordine delle parole perturbato.

    \item I probe mostrano alta accuratezza già nei layer medi di BERT
    e riconoscono in modo robusto la costruzione anche con pochi esempi di training.

    \item BERT è in grado di disambiguare i principali sottotipi semantici
    (SUCCESSION vs. JUXTAPOSITION)

    \item Le performance restano stabili anche con forti riduzioni dei dati:
    indizio che la conoscenza costruzionale è codificata latentamente nel modello.

    \item Evidenza a supporto dell’ipotesi che gli LLMs acquisiscano informazioni costruzionnali.
\end{itemize}
\end{frame}
